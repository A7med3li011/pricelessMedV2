...
interface MapSliderProps {
  ...
  onFacilitySelect: (coords: [number, number], facilityId?: string) => void;
  loading?: boolean;
        ...
        <div className="flex gap-4 pb-2">
          {facilities.map((facility) => {
            ...
            return (
              ...
              >
                <div
                  className={`bg-white rounded-lg shadow-md overflow-hidden h-full hover:shadow-xl transition-all ${
                    isSelected ? "border-4 border-[#8A44D9] shadow-2xl" : ""
                  }`}
                >
                  {/* Image Section */}
                  <div className="relative w-full h-[160px] bg-gray-100">
                    ...
                    {facility.estimatedMinutes && (
                      <div className="absolute top-2 left-2 bg-white text-black text-xs font-semibold px-2 py-1 rounded">
                        {facility.estimatedMinutes} mins aways
                      </div>
                    )}
                  </div>
